...
params = {'max_depth': 4, 'max_leaf_nodes':4}
cv= KFold(n_splits=10, shuffle=True)
for i, (train, test) in enumerate(cv.split(X_train, y_train)):
        model = DecisionTreeClassifier(**params).fit(X_train.iloc[train], y_train.iloc[train])
...
df2.to_csv("D:\\enen\\data\\ASUSdoucment\\python porject\\shaplast\\predtraintree10.csv", header=True, index=None)