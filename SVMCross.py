...
params = {'C': 1, 'kernel':'linear','gamma': 0.001}
cv= KFold(n_splits=10, shuffle=True)
for i, (train, test) in enumerate(cv.split(X_train, y_train)):
        model = SVC(**params, probability=True).fit(X_train.iloc[train], y_train.iloc[train])
...
y_train.to_csv("D:\\enen\\data\\ASUSdoucment\\python porject\\shaplast\\yTrainSVM.csv", header=True, index=None)
